...
def main():
        ...
        mlce = LightCurveClassifier(lr=6e-4, transformer_kwargs={"emb":128, "heads":4, "layers":2, "dropout_p":0.05, "hidden":512},optimizer_kwargs={}, num_classes=8, class_weights=class_weights)
        ...
        test_preds = mlce.test_preds
        test_labels = mlce.test_labels
        # print(mlce.test_step(test_loader,test_idx))
        #Make confusion matrix
        # preds = mlce(flux[test_idx], time[test_idx])
        # print(preds)
        # print(flux[test_idx].shape, time[test_idx].shape)
        # for input in flux[test_idx]:
        #      output = mlce(input)
        ...
        # print('a',test_labels)
        print(preds.device, 'p', test_labels.device)
        test_labels = test_labels.to('cpu')
        metric = MulticlassConfusionMatrix(num_classes=8).to('cpu')
        metric(preds, test_labels)
